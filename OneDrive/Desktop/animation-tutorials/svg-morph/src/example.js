 ...
 */
{
  ...
  // when you come onto the page, this is the infinite morphing of the svg at the top that just loops
  ...
  // this animation moves the svg to the initial position
  // literally as you come onto the page, hence why it has shapes[0]
  const initShapeEl = function () {
    ...
    // first move to first position then start doing infinite morph at the top
    // no pos argument below, so defaults to 0

    initShapeLoop();
  };

  const createScrollWatchers = function () {
    // forEach gives us access to each el (array item) and the pos (array index)
    ...
    DOM.contentElems.forEach((el, pos) => {
      ...
      //   when you reach the content item
      //   but with the offset
      ...
      watcher.enterViewport(function () {
        ...
        // this anime morphs from the one to the other
        // morphs to the initial state of the next scroll position
        ...
        anime({
          ...
          //   this is the next infinite morph loop of the initial part
        ...
        // this is the actual translation of the svg from one to the other
        // this is for afterwards when you start scrolling so pos is relevant now
      ...
      watcher.exitViewport(function () {
        //   when the watcher with the offset is above, change to pos + 1 otherwise pos-1
        const idx = !watcher.isAboveViewport ? pos - 1 : pos + 1;
        // lets say there is something above, then idx = 1 if pos =0
        // side note: if you went back up and there  wasn't anything above, then idx = 0 since pos = 1-1 =0
        // THEN below step == idx and the whole thing won't run but the entering viewport will run
